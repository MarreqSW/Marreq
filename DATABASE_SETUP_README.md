...
ReqMan now uses a single database strategy:
...
1. Create the database:
   ```bash
   docker compose exec -T db psql -U rust -d postgres -c "CREATE DATABASE reqman;"
   ```
...
3. Seed sample data:
   ```bash
   docker compose exec -T db psql -U rust -d reqman < scripts/init_complete.sql
   ```
...
- Creates the `reqman` database if absent
...
### `scripts/db_reset.sh`
Drops the `reqman` database entirely.  Development use only.

## Verification

```bash
# Check tables
docker compose exec -T db psql -U rust -d reqman -c "\dt"

# Check users
docker compose exec -T db psql -U rust -d reqman -c "SELECT username, name, is_admin FROM users ORDER BY id;"

# Check sample data
docker compose exec -T db psql -U rust -d reqman -c "SELECT COUNT(*) AS requirements FROM requirements;"
```
...
```bash
./scripts/db_backup.sh
# Saves to ./backups/reqman_<timestamp>.sql.gz
```

### Restore Backup
```bash
gunzip -c backups/reqman_<timestamp>.sql.gz | \
  docker compose exec -T db psql -U rust -d reqman
```
