...
//!
//! The actual route handlers (`POST /api/users`, `DELETE /api/users/<id>`) live
//! in [`super::users`] and are mounted unconditionally as part of core's base
//! route set.  The runtime guard
//! [`crate::deployment::current().allows_self_administered_user_creation()`]
...
//!
//! This module will become the home for any additional server-only admin
//! user-management handlers once the `marreq-server` crate split is complete.

#[cfg(test)]
mod tests {
    use crate::api::users::{create, delete, get, list};
    ...
    async fn client_with_repo(repo: DieselRepoMock) -> Client {
        // Set the deployment mode so that `deployment::current()` works inside
        // the `create` handler.  OnceLock silently ignores subsequent calls
        // with the same or different value, so this is safe across parallel tests.
        crate::deployment::set_current(crate::deployment::default_mode());
