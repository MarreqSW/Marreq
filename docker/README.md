...
| `ollama` | Published on host **`127.0.0.1:11435`** → container `11434` (avoids conflict with a local Ollama on **11434**). The Marreq container still uses `http://ollama:11434` on the Docker network. Omitted entirely by `docker-compose.light.yml`. |
...
Start the lightweight self-hosted stack without downloading or starting Ollama:

```bash
docker compose -f docker/docker-compose.light.yml up -d --build
```

`docker-compose.light.yml` is an alternative base file, not an override for `docker-compose.yml`. It forces `EMBEDDINGS_ENABLED=false` and `RAG_ENABLED=false`; see [LIGHT.md](LIGHT.md) for details.
...
For the light cloud variant, replace `docker-compose.yml` with `docker-compose.light.yml` in that command.
...
`OLLAMA_URL` stay in the full Compose file so they do not duplicate host-local
values in `.env`. The light stack does not configure or require Ollama.

## Production HTTPS deployment

Marreq does not need to terminate TLS inside Rocket. In production, run either the full or light split stack on loopback and terminate TLS in a host-level reverse proxy such as Caddy, nginx, or Traefik. The reverse proxy owns public ports **80/443** and forwards the HTTPS origin to the frontend on **127.0.0.1:8080**. The frontend nginx then serves the SPA and proxies `/api/` to Rocket over the internal Docker network.
...
For the full stack with Ollama, validate the merged Compose configuration first:
...
Both base stacks bind PostgreSQL (`5433`), Rocket (`8000`/`8001`), the SPA frontend (`8080`/`8082`), and Adminer (`8081`) to `127.0.0.1`. The full stack also binds Ollama (`11435`) to loopback; the light stack does not define Ollama at all.
...
Compose maps the DB to host **5433**, not 5432, so it should not collide with system PostgreSQL. If you still see bind errors, another local service may be using **5433** — change the loopback mapping in the selected base Compose file (e.g. `127.0.0.1:5434:5432`).

### Port 11434 already in use

This section applies only to the full `docker-compose.yml` stack. Compose maps Ollama to host **11435**, not 11434, so it should not collide with a host-installed Ollama. To call the **container** Ollama from your machine (e.g. `curl`), use `http://localhost:11435`. If **11435** is taken, change the loopback mapping (e.g. `127.0.0.1:11436:11434`).
...
For light deployments, replace `docker-compose.yml` with `docker-compose.light.yml` in these commands.
...
Usually means a frontend container started before its backend was listening (migrations/seed) or nginx had a **stale IP** for `marreq-server` / `marreq-cloud` after a recreate. Both base stacks use backend `healthcheck`s and `depends_on: condition: service_healthy` so nginx starts only after `GET /api/auth/csrf` succeeds on the target backend; nginx is configured with **Docker DNS resolver** + variable `proxy_pass` so upstream names are re-resolved. Rebuild the image and recreate the affected backend/frontend pair using the same base Compose file selected for the deployment.
