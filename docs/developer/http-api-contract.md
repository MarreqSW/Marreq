...
| `POST` | `/api/projects/{project_id}/imports/reqif` | Multipart field `file` (`.reqif` or `.xml`). Requires `EditRequirements`. Creates requirements using project catalog defaults (first status, category, applicability, verification method) and the current user as author/reviewer. Returns `imported_count`, `created_link_count`, `errors`, and `warnings`. Rejects ReqIFZ/ZIP. Send `X-CSRF-Token`. |
