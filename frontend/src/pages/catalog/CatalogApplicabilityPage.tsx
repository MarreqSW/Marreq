...
export default function CatalogApplicabilityPage() {
  ...
  async function addRow(e: FormEvent) {
    ...
    try {
      ...
      await createApplicability(body, token);
  ...
  async function saveRow(id: number) {
    ...
    try {
      ...
      await updateApplicability(c.id, body, token);
      await load();
    } catch (e) {
  ...
  async function removeRow(id: number) {
    if (!canEdit || !token) return;
    if (!window.confirm('Delete this applicability? Requirements may reference it.')) return;
    setBusy(true);
    setErr(null);
    try {
      await deleteApplicability(id, token);
      await load();
    } catch (e) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-stitch-border overflow-hidden bg-stitch-surface">
        ...
        {sorted.length === 0 && (
          <p className="p-6 text-center text-stitch-muted text-sm">
            No applicability rows for this project.
          </p>
        )}
      </div>
    </div>
