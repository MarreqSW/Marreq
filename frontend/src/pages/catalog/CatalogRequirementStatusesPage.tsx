...
import type { RequirementStatus, RequirementStatusWriteBody } from '@/api/types';
import { StatusBadge } from '@/components/StatusBadge';
import TagColorPicker from '@/components/TagColorPicker';
import { btnDanger, btnPrimary, inp } from './catalogUi';

export default function CatalogRequirementStatusesPage() {
  const { projectId: projectIdParam } = useParams();
  const pid = Number(projectIdParam);
  const { csrfToken } = useDashboard();
  const [rows, setRows] = useState<RequirementStatus[]>([]);
  ...
  async function addRow(e: FormEvent) {
    ...
    setErr(null);
    try {
      const body: RequirementStatusWriteBody = {
      ...
      await createRequirementStatus(body, token);
  ...
  async function saveRow(id: number) {
    ...
    try {
      ...
      await updateRequirementStatus(c.id, body, token);
      await load();
    } catch (e) {
  ...
  async function removeRow(id: number) {
    ...
    setErr(null);
    try {
      await deleteRequirementStatus(id, token);
      await load();
    } catch (e) {
