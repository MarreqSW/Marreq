...
#[get("/categories")]
pub fn list() -> ApiResult<Json<Vec<Category>>> {
...
#[get("/categories/<id>")]
pub fn get(id: i32) -> ApiResult<Json<Category>> {
    DieselCachedRepo::read()
...
pub fn create(payload: Json<NewCategory>) -> ApiResult<Value> {
...
pub fn update(id: i32, payload: Json<NewCategory>) -> ApiResult<Value> {
    let mut category = payload.into_inner();
    category.cat_id = Some(id);
    let before = DieselCachedRepo::read().get_category_by_id(id).ok();

    let updated = DieselCachedRepo::write()
    ...
    if let (Some(previous), Ok(mut conn)) =
        (before, DieselCachedRepo::read().inner_repo().get_conn())
    {
...
pub fn delete(id: i32) -> ApiResult<Status> {
    let category = DieselCachedRepo::write()
        .delete_category(id)
        .map_err(|err| match err {
            RepoError::NotFound => ApiError::NotFound(format!("category {id} not found")),
            other => other.into(),
