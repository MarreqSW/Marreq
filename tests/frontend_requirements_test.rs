...
async fn requirements_page_renders_correct_html_structure() {
    ...
    assert!(html.contains("reqman-requirements-page"), "Missing requirements page container");
    assert!(html.contains("reqman-requirements-header"), "Missing header");
    assert!(html.contains("reqman-requirements-table"), "Missing requirements table");
    assert!(html.contains("requirementsFilterForm"), "Missing filter form");
    assert!(html.contains("newRequirementButton"), "Missing new requirement button");
}
...
async fn requirements_page_displays_metrics_section() {
    ...
    assert!(html.contains("reqman-requirements-metrics"), "Missing metrics section");
...
async fn requirements_table_contains_sortable_headers() {
    ...
    assert!(html.contains("data-sort-key=\"key\""), "Missing sortable key column");
    assert!(html.contains("data-sort-key=\"title\""), "Missing sortable title column");
    assert!(html.contains("data-sort-key=\"status\""), "Missing sortable status column");
    assert!(html.contains("class=\"is-sortable\""), "Missing sortable class");
}
...
async fn requirement_row_has_correct_data_attributes() {
    ...
    assert!(html.contains("data-status-id"), "Missing status ID attribute");
    assert!(html.contains("data-status-label"), "Missing status label attribute");
    assert!(html.contains("data-action=\"toggle-row-details\""), "Missing toggle action");
    assert!(html.contains("data-action=\"duplicate-requirement\""), "Missing duplicate action");
}
...
async fn new_requirement_form_has_required_fields() {
    ...
    assert!(html.contains("name=\"req_description\""), "Missing description field");
    assert!(html.contains("name=\"req_reference\""), "Missing reference field");
    assert!(html.contains("name=\"req_category\""), "Missing category field");
    assert!(html.contains("name=\"req_verification\""), "Missing verification field");
    assert!(html.contains("name=\"req_current_status\""), "Missing status field");
    assert!(html.contains("name=\"req_applicability\""), "Missing applicability field");
    assert!(html.contains("name=\"req_justification\""), "Missing justification field");
    assert!(html.contains("data-requirement-form"), "Missing form marker");
}
...
async fn edit_requirement_form_populates_existing_data() {
    ...
    assert!(html.contains("REQ-SYS-001"), "Missing requirement reference");
    assert!(html.contains("value=\"REQ-SYS-001\"") || html.contains("REQ-SYS-001"), "Reference not in form");
    assert!(html.contains("data-requirement-form"), "Missing form marker");
    assert!(html.contains("data-allow-soft-mismatch"), "Missing validation config");
}
...
async fn requirement_detail_page_shows_relationships() {
    ...
    assert!(html.contains("requirement_data"), "Missing requirement data");
    assert!(html.contains("relationships") || html.contains("children"), "Missing relationships section");
}
...
async fn create_requirement_redirects_to_detail_page() {
    ...
    let response = client
        ...
        .private_cookie(session_cookie(1))
        .body("req_title=New+Req&req_description=Body&req_verification=1&\
    ...
    assert!(location.contains("/p/1/requirements/show/"), "Should redirect to detail page");
}
...
async fn create_requirement_with_add_another_redirects_to_form() {
    ...
    let response = client
        ...
        .header(ContentType::Form)
        .private_cookie(session_cookie(1))
        .body("req_title=Test&req_description=Body&req_verification=1&\
               req_current_status=1&req_reviewer=1&req_category=1&req_parent=0&\
    ...
    assert_eq!(response.status(), Status::SeeOther);
    let location = response.headers().get_one("Location").expect("redirect location");
    assert!(location.contains("/p/1/requirements/new"), "Should redirect to new form");
    assert!(location.contains("created=1"), "Should include success parameter");
}
...
async fn edit_requirement_redirects_to_detail_page() {
    ...
    let response = client
        ...
        .header(ContentType::Form)
        .private_cookie(session_cookie(1))
        .body("req_id=1&req_title=Updated&req_description=Body&req_verification=1&\
    ...
    assert_eq!(response.status(), Status::SeeOther);
    let location = response.headers().get_one("Location").expect("redirect location");
    assert!(location.contains("/p/1/requirements/show/1"), "Should redirect to detail page");
}
...
async fn delete_requirement_redirects_to_list() {
    ...
    assert_eq!(response.status(), Status::SeeOther);
    let location = response.headers().get_one("Location").expect("redirect location");
    assert!(location.contains("/p/1/requirements"), "Should redirect to requirements list");
}
...
async fn filter_form_submission_updates_url_parameters() {
    ...
    assert!(html.contains("action=\"/p/1/requirements\""), "Filter form action incorrect");
    // Verify selected options are marked
    assert!(html.contains("selected") || html.contains("value=\"1\""), "Filters not applied");
}
...
async fn requirements_page_includes_filter_controls() {
    ...
    assert!(html.contains("data-filter-control=\"status\""), "Missing status filter control");
    assert!(html.contains("data-filter-control=\"verification\""), "Missing verification filter control");
    assert!(html.contains("data-filter-control=\"category\""), "Missing category filter control");
    assert!(html.contains("data-action=\"clear-filters\""), "Missing clear filters button");
}
...
async fn requirements_table_has_search_input() {
    ...
    assert!(html.contains("id=\"requirementsSearch\""), "Missing search input");
    assert!(html.contains("type=\"search\""), "Search input wrong type");
    assert!(html.contains("name=\"search\""), "Search input missing name");
}
...
async fn requirement_form_has_reference_validation_markers() {
    ...
    assert!(html.contains("id=\"reference-error\""), "Missing reference error element");
    assert!(html.contains("id=\"req_reference\""), "Missing reference input");
    assert!(html.contains("data-role=\"submit-requirement\""), "Missing submit button marker");
}
...
async fn edit_form_has_autosave_configuration() {
    ...
    assert!(html.contains("data-role=\"autosave-text\"") || html.contains("autosave"), 
...
async fn requirements_page_displays_breadcrumb() {
    ...
    assert!(html.contains("Breadcrumb") || html.contains("breadcrumb"), "Missing breadcrumb");
    assert!(html.contains("Test Project"), "Missing project name in breadcrumb");
    assert!(html.contains("Requirements"), "Missing requirements in breadcrumb");
}
...
async fn new_requirement_form_displays_breadcrumb() {
    ...
    assert!(html.contains("/p/1/requirements"), "Missing back link to requirements");
}
...
async fn edit_requirement_form_displays_breadcrumb() {
    ...
    assert!(html.contains("breadcrumb"), "Missing breadcrumb");
    assert!(html.contains("/p/1/requirements"), "Missing requirements link");
    assert!(html.contains("REQ-SYS-001"), "Missing requirement reference");
}
...
async fn requirements_page_shows_action_buttons() {
    ...
    assert!(html.contains("New Requirement"), "Missing new requirement button");
...
async fn requirement_row_actions_have_correct_links() {
    ...
    assert!(html.contains("/p/1/requirements/edit/1"), "Missing edit link");
    assert!(html.contains("/p/1/requirements/show/1"), "Missing detail link");
    assert!(html.contains("data-requirement-id=\"1\""), "Missing requirement ID for actions");
}
